...
class AdminTermController extends PluginController{

    protected $tpl_model;
    protected $terms_model;
    protected $taxonomys=array("article"=>"文章","product"=>"产品","picture"=>"图片");
    ...
    function index(){
        ...
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        foreach ($result as $r) {
            $r['str_manage'] = '<a href="' . sp_plugin_url('VnimyExtend://AdminTerm/add', array("parent" => $r['term_id'])) . '">'.L('ADD_SUB_CATEGORY').'</a> | <a href="' . sp_plugin_url('VnimyExtend://AdminTerm/edit', array("id" => $r['term_id'])) . '">'.L('EDIT').'</a> | <a class="js-ajax-delete" href="' . sp_plugin_url('VnimyExtend://AdminTerm/delete', array("id" => $r['term_id'])) . '">'.L('DELETE').'</a> ';
    ...
    function add(){
        ...
        $tpl_list = $this->tpl_model->order('listorder asc')->select();
        ...
        if (IS_POST) {
            if ($this->terms_model->create()) {
                if ($this->terms_model->add()!==false) {
                    F('all_terms',null);
                    $this->success("添加成功！",sp_plugin_url('VnimyExtend://AdminTerm/index'));
                } else {
                    $this->error("添加失败！");
                }
            } else {
